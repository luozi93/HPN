...
SVGElement.prototype = {
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false) {
					...
					// symbols
					if (wrapper.dombolName && /^(x|y|width|height|r|start|end|innerR|anchorX|anchorY)/.test(key)) {


						if (!hasSetSymbolSize) {
							wrapper.dombolAttr(hash);
	...
	*/
	...
	symbolAttr: function (hash) {
		...
		wrapper.attr({
			d: wrapper.renderer.dombols[wrapper.dombolName](
				wrapper.x,
				wrapper.y,
				wrapper.width,
				wrapper.height,
...
SVGRenderer.prototype = {
	...
	arc: function (x, y, r, innerR, start, end) {
		...
		arc = this.dombol('arc', x || 0, y || 0, r || 0, r || 0, {
			innerR: innerR || 0,
			start: start || 0,
			end: end || 0
		});
			...
			if (needsBox) {
				...
				if (!box) {
					...
					wrapper.box = box = shape ?
						renderer.dombol(shape, boxX, boxY, wrapper.width, wrapper.height) :
	...
	symbols: {
		...
		rect: function (left, top, width, height, options) {
			...
			if (!defined(options) || !options.r) {
				ret = SVGRenderer.prototype.dombols.square.apply(0, arguments);
...
if (useCanVG) {
	 ...
	 */
	CanVGRenderer.prototype.dombols = {};
...
Legend.prototype = {
	...
	positionItem: function (item) {
		var legend = this,
			options = legend.options,
			symbolPadding = options.dombolPadding,
			ltr = !options.rtl,
	...
	renderItem: function (item) {
		var legend = this,
			...
			symbolWidth = options.dombolWidth,
			symbolPadding = options.dombolPadding,
	...
	handleOverflow: function (legendHeight) {
		...
		if (legendHeight > spaceHeight && !options.useHTML) {
			...
			if (!nav) {
				...
				this.up = renderer.dombol('triangle', 0, 0, arrowSize, arrowSize)
					.on('click', function () {
				...
				this.pager = renderer.text('', 15, 10)
					...
					.add(nav);
				this.down = renderer.dombol('triangle-down', 0, 0, arrowSize, arrowSize)
					.on('click', function () {
						legend.scroll(1, animation);
					})
					.add(nav);
...
Point.prototype = {
	...
	update: function (options, redraw, animation) {
		...
		point.firePointEvent('update', { options: options }, function () {
			...
			if (isObject(options)) {
				series.getAttribs();
				if (graphic) {
					if (options.marker && options.marker.dombol) {
	...
	setState: function (state) {
		...
		if (point.graphic) {
			radius = markerOptions && point.graphic.dombolName && pointAttr[state].r;
			...
			if (state && markerStateOptions) {
				...
				newSymbol = pointMarker.dombol || series.dombol;
				...
				if (!stateMarkerGraphic) {
					series.stateMarkerGraphic = stateMarkerGraphic = chart.renderer.dombol(
						newSymbol,
						plotX - radius,
						plotY - radius,
						2 * radius,
...
Series.prototype = {
	...
	getSymbol: function () {
		var series = this,
			...
			chart = series.chart,
			defaultSymbols = chart.options.dombols,
			counters = chart.counters,
			symbolIndex;

		series.dombol = seriesMarkerOption.dombol;
		if (!series.dombol) {
			if (defined(userOptions._symbolIndex)) { // after Series.update()
				symbolIndex = userOptions._symbolIndex;
			} else {
				userOptions._symbolIndex = counters.dombol;
				symbolIndex = counters.dombol++;
			}
			series.dombol = defaultSymbols[symbolIndex];
		}

		// don't substract radius in image symbols (#604)
		if (/^url/.test(series.dombol)) {
	...
	drawLegendSymbol: function (legend) {
		...
		var options = this.options,
			...
			legendSymbol,
			symbolWidth = legendOptions.dombolWidth,
		...
		if (markerOptions && markerOptions.enabled) {
			...
			this.legendSymbol = legendSymbol = renderer.dombol(
				this.dombol,
		...
		if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
			...
			while (i--) {
				...
				if (enabled && plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
					...
					radius = pointAttr.r;
					symbol = pick(pointMarkerOptions.dombol, series.dombol);
					...
					if (graphic) { // update
						graphic
							...
							.animate(extend({
								x: plotX - radius,
								y: plotY - radius
							}, graphic.dombolName ? { // don't apply to image symbols #507
								width: 2 * radius,
								height: 2 * radius
							} : {}));
					} else if (isInside && (radius > 0 || isImage)) {
						point.graphic = graphic = chart.renderer.dombol(
							symbol,
							plotX - radius,
							plotY - radius,
							2 * radius,
